...
int main(){
  ...
  //Cube
  vector<vector<float>> points{{-w/6,-w/6,-w/6},{w/6,-w/6,-w/6},{w/6,w/6,-w/6},{-w/6,w/6,-w/6},{-w/6,-w/6,w/6},{w/6,-w/6,w/6},{w/6,w/6,w/6},{-w/6,w/6,w/6}};
  ...
  while (delay < 10000){
    ...
    //connect edges (cube)
    for(int i=0; i < 4; i++){
      drawLine(image,pts[i],pts[(i+1)%4]);
      drawLine(image,pts[i+4],pts[((i+1)%4)+4]);
      drawLine(image,pts[i],pts[i+4]);
    }
